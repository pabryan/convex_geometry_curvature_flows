...
\partial_t g = k
\]
for \(k\) a symmetric \((0, 2)\)-tensor. Note that since the metric is symmetric, \(k\) must be symmetric: \(k(X, Y) = \partial_t (g(X, Y)) = \partial_t (g(Y, X)) = k(Y, X)\). When the variation of \(g_t\) is induced by the variation \(V\) of \(F_t\), \Cref{lem:g_t} gives us \(k(X, Y) = g((DV)^{\tang} (X), Y) + g(X, (DV)^{\tang} (Y))\).
...
\[
g(\partial_t (\nabla_t)_X Y, Z) = \frac{1}{2} \altcyc \sym \nabla_X g((DV)^{\tang} (Z), Y).
\]
\end{lemma}
...
\end{split}
\]
For the alternating, cyclic sum we have,
\begin{align*}
\partial_X k (Z, Y) + k ([X, Y], Z) &= \nabla_X k (Z, Y) + k(\nabla_X Z, Y) + k(Z, \nabla_X Y) + k([X, Y], Z) \\
&= \nabla_X k (Z, Y) + k(\nabla_X Z, Y) + 2 k(Z, \nabla_X Y) - k(\nabla_Y X, Z) \\
-\partial_Z k (Y, X) - k ([Y, Z], X) &= -\nabla_Z k (Y, X) - k(\nabla_Z Y, X) - k(Y, \nabla_Z X) - k([Y, Z], X) \\
&= -\nabla_Z k (Y, X) - k(Y, \nabla_Z X) - k(\nabla_Y Z, X) \\
\partial_Y k (X, Z) + k ([Z, X], Y) &= \nabla_Y k (X, Z) + k(\nabla_Y X, Z) + k(X, \nabla_Y Z) + k([Z, X], Y) \\
&= \nabla_Y k (X, Z) + k(\nabla_Y X, Z) + k(X, \nabla_Y Z) + k(\nabla_Z X, Y) -  k(\nabla_X Z, Y)
\end{align*}
